...
def create_barcode(entries):
   ...
   # dd_number : update_dd_number()
    ...
    svg.write('barcode417.svg')
    img = render_image(codes, ratio=4)
    img.save('barcode417.jpg')

    barcode128_data = entries['back_data_number_21'].get().strip()

    def generate_barcode128(data, output_file):
        code128 = barcode.get('code128', data, writer=SVGWriter())
        code128.save(output_file)

    generate_barcode128(barcode128_data, 'barcode128.svg')
def replace_barcode():
    barcodes = {
    "barcode128": {"id": "image1-1", "barcode_file": "barcode128.png"},
    "barcode417": {"id": "image1-3", "barcode_file": "barcode417.svg"}
     }

    for barcode in barcodes:

 # Find the existing barcode element by some identifier, for example, an id or specific tag
        existing_barcode = root.find(f'.//*[@id="{id}"]')

        if existing_barcode is not None:
            # Copy over the position and size attributes from the existing barcode
            x = existing_barcode.attrib.get('x', '0')
            y = existing_barcode.attrib.get('y', '0')
            width = existing_barcode.attrib.get('width', '100')
            height = existing_barcode.attrib.get('height', '50')

            # Remove the existing barcode element
            parent = root.find(f'.//*[@id="{id}"]')
            parent.remove(existing_barcode)

            # Create a new image element with the PNG barcode
            image_element = ET.Element('image', {
                'x': x,
                'y': y,
                'width': width,
                'height': height,
                'href': barcode_data_uri,
                'id': f'"{id}"'  # Ensure the new barcode has the same id
            })

            # Append the new image element to the parent
            parent.append(image_element)

            # Write the updated SVG back to a file
            tree.write(output_svg)
        else:
            print("Barcode element not found in the original SVG.")
...
def update_address_line_2(entries):
    city = entries['city'].get().strip()
    zip_code = entries['zip_code'].get().strip()
    address_line_2 = f"{city} WA {zip}".strip()

    # Find the tspan element for the full name and update it
    namespaces = {
        '': 'http://www.w3.org/2000/svg'
    }
    address_line_2_element = svg_tree.find('.//*[@id="tspan94"]', namespaces)  # Adjust ID if necessary
    if address_line_2_element is not None:
        address_line_2_element.text = address_line_2
...
def on_submit(entries):
   ...
   # print("Formatted Height:", formatted_height)
    height = height_convert()
   # height_convert(entered_height)
    update_dd_number()
    get_city_zip()
    #update_address_line_2(entries)
...
fields = {
    ...
    "back_data_number_21": {"id": "tspan17" , "type": "text", "row": 16},
...
submit_btn.grid(row=22, column=1)
create_barcode(entries)
...
update_barcode_image_photo_button = tk.Button(app, text="update barcode",
                                            command=lambda: load_image(barcode_image_photo_label, 'barcode417.jpg'))
